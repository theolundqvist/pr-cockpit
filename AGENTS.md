...
- Before blaming GitHub for a slow refresh or poll, time its SQLite reads on a copy of the real cache. The cache has no `ANALYZE` statistics, so SQLite often prefers an index that satisfies `ORDER BY` over a selective one and walks a whole repository's Actions rows; make the selective index also cover the ordering, and pin join order with `CROSS JOIN` where needed. Benchmark server paths from a `.scratch` script that imports the modules with `COCKPIT_DATA_DIR` on the copy and stubs `fetchMirror`, rather than a full server that starts relay, webhooks, and forwarders.
- Background UI preloads call per-PR GETs with `prefetch=1`, which serves only SQLite and local mirror data and answers `204` on a miss; interactive opens alone may reach GitHub, so preloading every inbox PR costs no quota. Under live relay or forwarder coverage a stored detail refetches only after a newer webhook for that PR (10-minute safety net).
- Measure the Electron shell as a fresh process: a `.scratch` entry that sets its own `userData` and a remote-debugging port, records `BrowserWindow` events, then requires `shell/main.js`. A Playwright CDP attachment keeps the page visible, so a hidden window never fires `visibilitychange` or throttles timers; measure hide/show behaviour with a session preload that reports over IPC, not over CDP. Chromium runs throttled timers as soon as the window is shown.
...
- Sleep leaves remote sockets half-open with no close event, so every long-lived network wait needs its own liveness check or deadline (relay pings, request timeouts, git low-speed limits). Rehearse a wake on a scratch server with `kill -STOP <pid>`, wait past 30 seconds, then `kill -CONT <pid>`: a poll should complete within seconds and the relay should reopen.
